...
class Transport:
    ...
    async def _control_loop(
        ...
        *,
        ...
        """Open the home ``SubscribeControl`` stream and route its events.

        Called once from :meth:`_serve` for the Client's home sub. Events
        are dispatched through ``_control_forward`` (goldfive bridge
        path) or ``_dispatch_control`` (per-kind handler path).
        """
        from goldfive.pb.goldfive.v1 import control_pb2 as gf_control_pb2
