...
class TestGoldfiveRoundTrip:
    ...
    @pytest.mark.asyncio
    async def test_ghost_home_session_hidden_from_list_sessions(
        self,
        harmonograf_server: dict,
    ) -> None:
        """End-to-end reproducer for harmonograf#77.

        The ``Client.__init__`` flow sends a ``Hello`` with an empty
        ``session_id``, triggering the server to auto-generate a
        ``sess_YYYY-MM-DD_NNNN`` home session. Under goldfive+adk-web,
        every subsequent span and goldfive event carries its own
        ``session_id`` (the outer adk-web session), so the home row sits
        with zero spans and zero plans — pure picker clutter.

        This test asserts that ``ListSessions`` hides the ghost while
        continuing to surface the content-bearing session.
        """
        ...
        try:
            # Emit one span stamped with an explicit session_id the way
            # HarmonografTelemetryPlugin does under adk-web. The Hello
            # was already sent with an empty session_id → server
            # auto-created a ghost ``sess_*`` home session. This span
            # will land on ``orch_ghost_test`` via ``_ensure_route``.
            sid = client.emit_span_start(
                kind="TOOL_CALL",
                name="echo",
                session_id="orch_ghost_test",
            )
            client.emit_span_end(sid, status="COMPLETED")

            # Wait for ingest to land the span on the server.
            store = harmonograf_server["store"]
            deadline = asyncio.get_event_loop().time() + 5.0
            spans: list[Any] = []
            while asyncio.get_event_loop().time() < deadline:
                spans = await store.get_spans("orch_ghost_test")
                if spans:
                    break
                await asyncio.sleep(0.05)
            assert spans, "span never landed on orch_ghost_test"

            # Confirm both rows exist in the store (ghost home +
            # content-bearing target session).
            all_sessions = await store.list_sessions()
            ids = {s.id for s in all_sessions}
            assert "orch_ghost_test" in ids
            ghost_ids = {sid for sid in ids if sid.startswith("sess_")}
            assert ghost_ids, (
                "expected at least one auto-generated home session, "
                f"got {ids}"
            )

            # ListSessions (default — include_empty=False) must hide the
            # ghost and surface only ``orch_ghost_test``.
            async with grpc.aio.insecure_channel(harmonograf_server["addr"]) as ch:
                stub = service_pb2_grpc.HarmonografStub(ch)
                resp = await stub.ListSessions(
                    frontend_pb2.ListSessionsRequest(limit=50)
                )

            picker_ids = {s.id for s in resp.sessions}
            assert "orch_ghost_test" in picker_ids
            assert ghost_ids.isdisjoint(picker_ids), (
                f"ghost session(s) {ghost_ids & picker_ids} leaked into picker"
            )

            # include_empty=True brings the ghost back for admin tools.
            async with grpc.aio.insecure_channel(harmonograf_server["addr"]) as ch:
                stub = service_pb2_grpc.HarmonografStub(ch)
                admin = await stub.ListSessions(
                    frontend_pb2.ListSessionsRequest(limit=50, include_empty=True)
                )
            admin_ids = {s.id for s in admin.sessions}
            assert ghost_ids.issubset(admin_ids), (
                f"include_empty=True must surface ghost sessions: "
                f"admin_ids={admin_ids} ghost_ids={ghost_ids}"
            )
