...
**Superseded by [ADR 0011](0011-reporting-tools-over-span-inference.md).** This ADR records the decision as it stood in
...
is also listed in [ADR 0011](0011-reporting-tools-over-span-inference.md) as the motivation for the pivot.
...
- **Parallel races.** When the DAG walker was introduced for parallel
  mode ([ADR 0012](0012-three-orchestration-modes.md)), span-close callbacks fired in arbitrary order across
...
[ADR 0011](0011-reporting-tools-over-span-inference.md) describes the replacement: explicit reporting tools intercepted
